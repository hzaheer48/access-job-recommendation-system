...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    if (state.isAuthenticated) {
      const redirectPath =
        state.user?.role === 'admin' ? '/admin/dashboard' : '/dashboard';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const user = mockUsers.find(
        (u) =>
          u.email.toLowerCase() === formData.email.toLowerCase() && u.isActive
      );
                ...
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <div className='text-xs text-blue-600'>
                      {mockUsers.find((u) => u.role === 'job_seeker')?.email}
                    </div>
                  </div>
                  <svg
                ...
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <div className='text-xs text-purple-600'>
                      {mockUsers.find((u) => u.role === 'admin')?.email}
                    </div>
                  </div>
                  <svg
                    className='w-4 h-4 text-purple-600 group-hover:translate-x-1 transition-transform duration-200'
                    fill='none'
                    stroke='currentColor'
                    viewBox='0 0 24 24'
                  >
